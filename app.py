...
mother_education = st.sidebar.selectbox("Educacion de la Madre", [1, 2, 3, 4], format_func=lambda x: ["Sin estudios", "Primaria", "Bachillerato", "Profesional"][x-1])
father_education = st.sidebar.selectbox("Educacion de el Padre", [1, 2, 3, 4], format_func=lambda x: ["Sin estudios", "Primaria", "Bachillerato", "Profesional"][x-1])
travel_time = st.sidebar.selectbox("Tiempo de desplazamiento", [1, 2, 3, 4], format_func=lambda x: ["< 15 min", "15-30 min", "30-60 min", "> 1 hora"][x-1])
...
number_of_absences = st.sidebar.slider("Numero de Fallas", 0, 32, 5)
final_grade = st.sidebar.slider("Nota Final", 0, 5, 3)
address = st.sidebar.selectbox("Direccion", ["U", "R"], format_func=lambda x: "Urbana" if x == "U" else "Rural")
